...
impl Term {
    pub(crate) fn type_in(&self, ctx: &Context<Binding>) -> Result<Ty> {
        match self {
            ...
            TyAbs(name, t2) => {
                let mut inner = ctx.clone();
                inner.add(name.clone(), Binding::TyVar);
                let ty2 = t2.type_in(&inner)?;
                Ok(Ty::All(name.clone(), ty2.boxed()))
            }
            TyApp(t1, ty2) => {
                let ty1 = t1.type_in(ctx)?;
                match ty1 {
                    Ty::All(_, ty12) => Ok(ty12.subst_top(ty2.clone())),
                    _ => bail!(
    ...
    fn eval1(self, ctx: &Context<Binding>) -> Term {
        match self {
            ...
            TyApp(t1, ty2) => match *t1 {
                TyAbs(_, t12) => t12.subst_type_top(ty2),
            ...
            Var(_, _) | Abs(_, _, _) | TyAbs(_, _) | Bool(_) | Nat(_) => unreachable!("no rule"),
        }
    }

    fn is_val(&self) -> bool {
        match self {
            Abs(_, _, _) | TyAbs(_, _) | Bool(_) | Nat(_) => true,
        ...
        F2: FnOnce(Ty, usize) -> Ty + Clone,
    {
        match self {
            ...
            TyAbs(name, t2) => TyAbs(name, t2.map(on_var, on_type, c + 1).boxed()),
    ...
    pub(crate) fn to_string_in(&self, ctx: &Context<()>) -> String {
        match self {
            ...
            TyAbs(name, t2) => {
                let mut inner = ctx.clone();
                inner.add_name(name.clone());
                format!("\\\\{name}. {}", t2.to_string_in(&inner))
            }
